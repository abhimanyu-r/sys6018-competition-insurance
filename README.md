...
## SVM
 ...
 We also used was Support Vector Machines (SVM). Due to performance constraints, the training data was made into a stratified sample of 10,000 observations. This helped boost the performance enough to rapidly construct models and made cross validation feasible. Two kernels were investigated in constructing the model. The first was a polynomial kernel given a degree of 3 (a standard recommended in the ISLR textbook). Cross-validating the polynomial model using a 10-fold CV we got an error estimation of 0.03550885. We also ran an SVM model with a radial kernel with a gamma set to 1. Upon completing 10 fold cross validation we attained an error estimation of 0.03528487. Thus, we were able to see that both kernels were performing comparably, but with the radial one slightly outperforming. THe radial model was, however, more computationally taxing. Nonetheless, we sought to make models of both.
 
 We ran a polynomial model with a sample size of 10,000, another polynomial kernel with a sample size of 50,000, and a radial kernel with a sample size of 50,000. The polynomial did not perform well with the test data. The 10,000 sample size model yielded a low score of .029 while a 50,000 sample yielded .036. These results did illustrate a major drawback that computational limits have imposed on us. Upping the stratified samples sized yields improvements in performance. Larger sample sizes would yield better results. The radial kernel performed significantly better 0.093. It also, however, was more computationally taxing and took longer to run. Nonetheless, we can speculate that larger sample sizes would improve this model, though more computational power would be needed to run them quickly. 
